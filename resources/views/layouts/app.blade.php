        ...
        <nav class="hidden md:flex items-center gap-5 text-sm">
            ...
            @auth
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button class="text-white hover:text-red-400 transition">Iziet</button>
                </form>
            @endauth
            @guest
    ...
    <nav id="mobile-nav"
        ...
        @auth
            ...
            <form method="POST" action="{{ route('logout') }}" class="pt-1">
                @csrf
                <button class="w-full text-left px-4 py-3 rounded-xl hover:bg-red-500/20 text-red-400 transition">Iziet</button>
            </form>
        @endauth
        @guest
            <a href="{{ route('login') }}"
